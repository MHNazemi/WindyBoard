...
class MC(IAi.IAi):
    ...
    def apply(self, reward=None):
        ...
        if r > 1/self.k:
            ...
            valMax = self.Q[self.states[str(x)+str(y)]][0]
            candidates = []
            for arg in args:
            ...
            selectedArg = random.choice(range(0, len(candidates)))
            action = self.reverseActions[selectedArg]

        else:
            selectedArg = random.randint(0, len(self.a)-1)
            action = self.reverseActions[selectedArg]
